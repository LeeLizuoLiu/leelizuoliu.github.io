...
function area(d){
 ...
 $('area-diagram').innerHTML=`<title>Nested areas for α = ${fmt(d.alpha)} radians</title><polygon points="${O} ${A} ${T}" fill="#dce8ff" stroke="#2563eb" stroke-width="2"/><path d="M ${O} L ${A} A ${r} ${r} 0 0 0 ${P} Z" fill="#b4e4d9" stroke="#087c78" stroke-width="2"/><polygon points="${O} ${A} ${P}" fill="#f9ce96" stroke="#bd5709" stroke-width="2"/><path d="M ${ax} ${oy} A ${r} ${r} 0 0 0 ${ox} ${oy-r}" fill="none" stroke="#52647c" stroke-width="2"/>${line(ox-15,oy,ax+80,oy,'#8193a9',1)}${line(ox,oy+12,ox,40,'#8193a9',1)}${line(ax,oy+12,ax,38,'#2563eb',1,'stroke-dasharray="5 5"')}${dot(ox,oy,'#172b49')}${dot(ax,oy,'#172b49')}${label(ox-20,oy+24,'O')}${label(ox+80,oy+26,'radius 1')}${label(ax+10,30,'x = 1')}${label(ox-30,68,'1')}`;
 if(d.h===0){$('area-diagram').innerHTML+=label(ax-15,oy-15,'A = P = B');}
 else{
  $('area-diagram').innerHTML+=`${dot(px,py,'#bd5709')}${dot(ax,ty,'#2563eb')}${label(ax+10,oy+24,'A')}${line(px,py,px-25,py-28,'#bd5709',1)}${label(px-45,py-34,'P')}${line(ax,ty,ax+35,ty-10,'#2563eb',1)}${label(ax+40,ty-7,'B')}${label(ox+44,oy-12,'α')}`;
 }
...
function graph(id,x0,x1,y0,y1,xticks,yticks){
 ...
 let out='';
 for(const [x,t]of xticks)out+=line(sx(x),40,sx(x),290,'#e3eaf3',1)+label(sx(x),315,t,'text-anchor="middle"');
 for(const y of yticks)out+=line(52,sy(y),468,sy(y),'#e3eaf3',1)+label(43,sy(y)+5,String(y),'text-anchor="end"');
 out+=line(52,sy(0),468,sy(0),'#a0adbd',1)+line(sx(0),40,sx(0),290,'#a0adbd',1);
 $(id).innerHTML=out;
...
function limits(d){
 ...
 g.path(Math.cos,'#8794a6',true);g.path(()=>1,'#8794a6',true);g.path(sinc,'#2563eb');g.path(x=>-Math.sin(x/2)*sinc(x/2),'#bd5709');
 g.add(label(475,335,'h')+dot(g.sx(0),g.sy(1),'#2563eb',6,'white')+dot(g.sx(0),g.sy(0),'#bd5709',6,'white'));
 if(d.h!==0)g.add(line(g.sx(d.h),40,g.sx(d.h),290,'#8897ab',1,'stroke-dasharray="4 4"')+dot(g.sx(d.h),g.sy(d.A),'#2563eb')+dot(g.sx(d.h),g.sy(d.B),'#bd5709'));
...
function slopes(d){
 ...
 g.add(line(g.sx(lo),g.sy(tangent(lo)),g.sx(hi),g.sy(tangent(hi)),'#087c78',3,'stroke-dasharray="6 4"'));
 if(d.h!==0){const sec=x=>d.value+d.quotient*(x-d.theta);g.add(line(g.sx(Math.min(lo,d.theta+d.h)),g.sy(sec(Math.min(lo,d.theta+d.h))),g.sx(Math.max(hi,d.theta+d.h)),g.sy(sec(Math.max(hi,d.theta+d.h))),'#bd5709',2)+dot(g.sx(d.theta+d.h),g.sy(f(d.theta+d.h)),'#bd5709')+label(g.sx(d.theta+d.h)+9,g.sy(f(d.theta+d.h))-13,'Q'));}
 g.add(dot(g.sx(d.theta),g.sy(d.value),'#2563eb')+label(g.sx(d.theta)-20,g.sy(d.value)+25,d.h===0?'P = Q':'P')+label(475,335,'x'));
